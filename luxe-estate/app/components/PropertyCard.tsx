...
export default function PropertyCard({ property }: PropertyCardProps) {
  ...
  return (
    <article className={`bg-white dark:bg-white/5 rounded-xl overflow-hidden shadow-card hover:shadow-soft transition-all duration-300 group cursor-pointer h-full flex flex-col ${property.visibilityClasses || ''}`}>
      <div className="relative aspect-[4/3] overflow-hidden">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img alt={property.imageAlt} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" src={property.image} />
        ...
        >
          <span className="material-icons text-lg">{favorite ? 'favorite' : 'favorite_border'}</span>
        </button>
        
        {property.status && (
          <div className={`absolute bottom-3 left-3 text-white text-xs font-bold px-2 py-1 rounded ${isForRent ? 'bg-mosque/90' : 'bg-nordic-dark/90'}`}>
